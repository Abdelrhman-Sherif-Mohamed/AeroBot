...
public static class NavMeshManager
{
    ...
    public static bool Raycast(NavMeshTransform src, NavMeshTransform dst, NavMeshRaycastType type, [NotNullWhen(true)] out NavMeshRaycastHit? hit)
    {
        ...
        {
            Debug.WriteLine($"NavMesh Raycast exception: {ex.Message}");
            hit = null;
            return true;
        }
    }
...
#pragma warning restore RCS1089 // Use --/++ operator instead of assignment.
    ...
    public static bool TryGetNavMeshObj(int index, out NavMeshObj obj)
    {
        if (!_objectCache.TryGetValue(index, out obj))
        {
            ...
            // Object file may be missing/renamed in some clients: skip it
            // instead of crashing the whole region load.
            var loaded = LoadNavMeshObj(objectIndex.Path);
            if (loaded == null)
            {
                obj = null;
                return false;
            ...
            loaded.IndexFlag = objectIndex.Flag;
            _objectCache[index] = obj = loaded;
        }
