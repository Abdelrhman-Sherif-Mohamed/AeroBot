...
public partial class SplashScreen : UIWindow
{
    ...
    private void SplashScreen_Load(object sender, EventArgs e)
    {
        ...
        Kernel.Language = GlobalConfig.Get("RSBot.Language", "ar_AR");
    ...
    private void ReferenceDataLoaderCompleted(object sender, RunWorkerCompletedEventArgs e)
    {
        ...
        // Silent boot for SonicMini: straight to the system tray, no big window.
        if (Program.StartMinimized)
        {
            var traySetting = GlobalConfig.Get("RSBot.General.TrayWhenMinimize", false);
            try
            {
                GlobalConfig.Set("RSBot.General.TrayWhenMinimize", true);
                _mainForm.WindowState = FormWindowState.Minimized;
            }
            finally
            {
                GlobalConfig.Set("RSBot.General.TrayWhenMinimize", traySetting);
            }
        }
    ...
    {
        var loadingText = LanguageManager.GetLang("Loading");
        lblLoading.Text = (string.IsNullOrEmpty(loadingText) ? "Loading" : loadingText) + $": {e.UserState}";
