...
{
    private static void DrawText(Graphics g, string text, Font font, Color color, RectangleF rect, StringFormat format)
    {
        if (string.IsNullOrEmpty(text) || rect.Width <= 0 || rect.Height <= 0)
            return;

        // Ensure proper RTL boundary for Arabic text so trailing/leading brackets, colons and numbers are not flipped
        if (HasArabic(text))
        {
            if (!text.StartsWith("\u200F"))
                text = "\u200F" + text;
    ...
    public static void DrawString(this Control control, string text, Graphics graphics, ContentAlignment contentAlignment, bool showEllipsis = false, bool useMnemonic = false, bool noWrap = false)
    {
        graphics.TextRenderingHint = TextRenderingHint.ClearTypeGridFit;
        using var textFormat = control.CreateStringFormat(contentAlignment, showEllipsis, useMnemonic);
        if (noWrap)
            textFormat.FormatFlags |= StringFormatFlags.NoWrap;
        DrawText(graphics, text, control.Font, control.ForeColor, control.ClientRectangle, textFormat);
    }

    public static void DrawString(this Control control, Graphics graphics, ContentAlignment contentAlignment, bool showEllipsis = false, bool useMnemonic = false, bool noWrap = false)
    {
        graphics.TextRenderingHint = TextRenderingHint.ClearTypeGridFit;
        using var textFormat = control.CreateStringFormat(contentAlignment, showEllipsis, useMnemonic);
        if (noWrap)
            textFormat.FormatFlags |= StringFormatFlags.NoWrap;
        DrawText(graphics, control.Text, control.Font, control.ForeColor, control.ClientRectangle, textFormat);
    }

    public static void DrawString(this Control control, Graphics graphics, ContentAlignment contentAlignment, Color color, bool showEllipsis = false, bool useMnemonic = false, bool noWrap = false)
    {
        graphics.TextRenderingHint = TextRenderingHint.ClearTypeGridFit;
        using var textFormat = control.CreateStringFormat(contentAlignment, showEllipsis, useMnemonic);
        if (noWrap)
            textFormat.FormatFlags |= StringFormatFlags.NoWrap;
        DrawText(graphics, control.Text, control.Font, color, control.ClientRectangle, textFormat);
    }

    public static void DrawString(this Control control, Graphics graphics, ContentAlignment contentAlignment, Color color, RectangleF rectangle, bool showEllipsis = false, bool useMnemonic = false, bool noWrap = false)
    {
        graphics.TextRenderingHint = TextRenderingHint.ClearTypeGridFit;
        using var textFormat = control.CreateStringFormat(contentAlignment, showEllipsis, useMnemonic);
        if (noWrap)
            textFormat.FormatFlags |= StringFormatFlags.NoWrap;
        DrawText(graphics, control.Text, control.Font, color, rectangle, textFormat);
    }
    ...
    public static void DrawString(this Control control, Graphics graphics, string text, Color color, RectangleF rectangle)
    {
        ...
        DrawText(graphics, text, control.Font, color, rectangle, textFormat);
    }
